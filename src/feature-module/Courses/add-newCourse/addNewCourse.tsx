...
const AddNewCourse: React.FC = () => {
  ...
  const handleAddLesson = (e?: React.FormEvent) => {
    if (e) e.preventDefault();
    if (
      !currentLesson.name.trim() ||
      (!currentLesson.vdoId.trim() && !currentLesson.videoUrl.trim())
    )
      return;
  ...
  const handleSubmitCourse = async (e: React.FormEvent) => {
    ...
    if (createCourse.fulfilled.match(resultAction)) {
      ...
      navigate(all_routes.adminCourses);
    } else {
      ...
      <form onSubmit={handleSubmitCourse}>
        <div className="content">
          <div className="container mb-10">
            <div className="mb-4">
              <Link to={all_routes.adminCourses} className="back-to-course">
