...
const signupStudent = createAsyncThunk<SignupResponse, SignupRequest>(
  ...
  async (data, thunkAPI) => {
    try {
      // Admin-created students go through the same signup flow (created disabled).
...
// ENABLE / DISABLE STUDENT (admin)
const setStudentStatus = createAsyncThunk<
  { id: string; isDisable: boolean },
  { id: string; isDisable: boolean }
>("student/setStatus", async ({ id, isDisable }, thunkAPI) => {
  try {
    await axios.patch(`${Base_URL}/${id}/status`, { isDisable });
    return { id, isDisable };
  } catch (err: any) {
    return thunkAPI.rejectWithValue(
...
// ASSIGN COURSES TO STUDENT (admin)
const enrollStudentCourses = createAsyncThunk<
  { id: string; enrolledCourses: string[] },
  { id: string; enrolledCourses: string[] }
>("student/enroll", async ({ id, enrolledCourses }, thunkAPI) => {
  try {
    const res = await axios.post(`${Base_URL}/${id}/enroll`, { enrolledCourses });
    return { id, enrolledCourses: res.data.enrolledCourses || enrolledCourses };
  } catch (err: any) {
    return thunkAPI.rejectWithValue(
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(setStudentStatus.fulfilled, (state, action) => {
        const { id, isDisable } = action.payload;
        state.students = state.students.map((s: any) =>
          s._id === id ? { ...s, isDisable } : s
        );
        if (
          state.profile &&
          (state.profile as any)._id === id &&
          state.profile.student
        ) {
          state.profile.student.isDisable = isDisable;
        }
      })
      // ENROLL
      .addCase(enrollStudentCourses.fulfilled, (state, action) => {
        if (state.profile && (state.profile as any)._id === action.payload.id) {
          state.profile.enrolledCourses = action.payload.enrolledCourses;
        }
      });
  },
});
